...
man_data <- man_data %>% 
  rename_at(vars(colnames(man_data)), ~ c("id", "boro", "occur_date", "occur_time",
                            ...
                            "report_date", "susp_age", "susp_race", "susp_sex", "vic_age", "vic_race", "vic_sex", "latitude", "logitude", "lat_lon")) %>% 
...
p[[1]] <- ggplot(aes(x = level, fill = level), data = man_data) + 
  geom_bar() +
  labs(x = "Crime level", y = "Number")

p[[2]] <- ggplot(aes(x = success_fail, fill = success_fail), data = 
                  man_data)+
    geom_bar()+
  labs(x = "The crime completed or not", y = "Number")
